...
server.route({
  ...
  method: 'GET',
  handler: function(request, reply) {
    reply("Oh, hi. You should POST to this address from Slack");
  }
});
...
var handleSlackInput = function(payload) {
  if (payload.user_id === 'USLACKBOT')
    return;
  if (ircClient) {
    if (payload.trigger_word !== undefined)
    ...
    var isCommand = true;
    if (config.slack.specifyBotForCmds) {
      if (words[0] !== config.slack.botName)
        isCommand = false;
    }
    ...
    if (isCommand && _.has(slackBotMethods, command)) {
      var response = slackBotMethods[command](payload);
      if (response) {
        sendToSlack(response);
      }
    } else {
      var messageText = words.join(' ');
      if (payload.user_id !== 'USLACKBOT') {
        var composed = "<" + payload.user_name + "> " + messageText;
        ircClient.say(config.irc.channel, composed);
      }
    }
  }
